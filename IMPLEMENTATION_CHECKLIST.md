...
- [ ] TC-REQ-006-5
- [ ] TC-REQ-006-6
- [ ] TC-REQ-007-1 — spec written (`e2e/tests/req-001-import.spec.ts`), discoverable/runs, fails today because `/corridors/new` doesn't exist yet (expected Loop A state — will check off in Loop B4 once it actually passes)
- [ ] TC-REQ-007-2 — spec written (`e2e/tests/req-002-manual-trace.spec.ts`), discoverable/runs, fails today because `/corridors/new` doesn't exist yet (expected Loop A state — will check off in Loop B4 once it actually passes)
- [ ] TC-REQ-007-3 — spec written (`e2e/tests/graceful-degradation.spec.ts`), discoverable/runs, fails today because `/corridors/new` doesn't exist yet (expected Loop A state — will check off in Loop B4 once it actually passes)
- [ ] TC-REQ-007-4 — spec written (`e2e/tests/req-005-reorder.spec.ts`), discoverable/runs, fails today because no seeded fixture/route exists yet (expected Loop A state — will check off in Loop B4 once it actually passes)
...
- **A4 (confirm compilation) result, all 7 Loop A groups:** `cargo build --workspace` clean (only expected dead-code warnings on unwired stub handlers/structs). `cargo nextest run --workspace --no-fail-fast` (with `DOCKER_HOST` set): **433 tests run, 369 passed, 64 failed, 4 skipped, 0 leaked-beyond-expected.** Confirmed via grep that **zero** failures are outside `corridor_design`/`corridor_import`/`web::corridor_design` modules — every pre-existing test still passes (no regressions), and all 64 failures are `unimplemented!()` panics in the new stub code, exactly the expected Loop A end-state. The 4 skipped are the documented `#[ignore]`d placeholders (TC-REQ-001-5, TC-REQ-002-06, TC-REQ-003-5, TC-REQ-005-6). On the Playwright side: `feature-detection.spec.ts` (9/9) passes for real (no app dependency); the four app-facing specs fail with a real `404`/timeout against a running server, confirming they fail for the right reason (routes not wired) rather than a bug in the test code. Loop A is complete; proceeding to Loop B.
