...
interface StopJpaRepository : JpaRepository<StopEntity, String> {
     ...
     */
    @Query("SELECT s FROM StopEntity s WHERE s.feedOnestopId = :feedId AND s.active = true ORDER BY s.name ASC")
     ...
     */
    @Query("SELECT s FROM StopEntity s WHERE s.feedOnestopId = :feedId AND s.gtfsStopId = :gtfsStopId")
     ...
     */
    @Query("SELECT s FROM StopEntity s WHERE s.feedOnestopId = :feedId AND s.locationType = :locationType ORDER BY s.name ASC")
     ...
     */
    @Query("SELECT s FROM StopEntity s WHERE s.feedOnestopId = :feedId AND s.locationType = 1 ORDER BY s.name ASC")
     ...
     */
    @Query("SELECT CASE WHEN COUNT(s) > 0 THEN true ELSE false END FROM StopEntity s WHERE s.feedOnestopId = :feedId AND s.gtfsStopId = :gtfsStopId")
