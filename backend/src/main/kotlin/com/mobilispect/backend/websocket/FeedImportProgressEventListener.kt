 ...
 *
 * This component replaces the imperative ProgressTrackingService by using Spring's
 * event-driven architecture. It automatically tracks import progress and broadcasts
 * updates to WebSocket subscribers when feed import events are published.
 *
 ...
 */
@Component
class FeedImportProgressEventListener(
    ...
    fun onFeedImportStarted(event: FeedImportStartedEvent) {
        ...
        stepTracking[importId] = StepTracking(
    ...
    fun onFeedImportStepStarted(event: FeedImportStepStartedEvent) {
        ...
        val updatedProgress = currentProgress.copy(
            ...
            estimatedTimeRemainingSeconds = calculateEstimatedTimeRemaining(
                tracking.startedAt,
                progressPercentage
            )
        )
    ...
    fun onFeedImportFailed(event: FeedImportFailedEvent) {
        ...
        logger.error("Feed import failed for feed: {}, step: {}, message: {}", importId, event.step, event.message)
    ...
    private fun broadcastProgress(importId: String, progress: ImportProgress) {
        ...
        logger.debug("Broadcast progress update for import: {}, progress: {}%", importId, progress.progressPercentage)
    }

    /**
     * Estimate total number of steps in a feed import.
     * This is a heuristic based on typical import jobs.
     */
