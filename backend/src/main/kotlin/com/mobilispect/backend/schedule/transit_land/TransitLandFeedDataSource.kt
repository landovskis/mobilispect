 ...
 *
 * Queries the Transit.Land v2 REST API for GTFS feeds matching the specified region. Uses the
 * /feeds.json endpoint with search parameter for direct feed discovery.
 */
class TransitLandFeedDataSource(
  ...
  override fun feeds(region: String): Collection<Result<ScheduledFeed>> {
    ...
    return transitLandClient
      .feeds(apiKey = apiKey, search = searchTerm, spec = "gtfs")
      .onSuccess { feeds ->
        logger.info(
          "Found {} GTFS feeds for region '{}' (search: '{}')",
          feeds.size,
          region,
          searchTerm,
        )
      ...
      .onFailure { e ->
        logger.error("Failed to query feeds for region '{}': {}", region, e.message)
      }
