...
/**
 * A client to access the transitland API with rate limiting. Rate limited to 6 requests per second
 * per Transit.land API documentation.
 */
@OptIn(ExperimentalSerializationApi::class)
@Component
class TransitLandClient(builder: WebClient.Builder, rateLimiterRegistry: RateLimiterRegistry) :
  TransitLandAPI {
  ...
  override fun feed(apiKey: String, feedID: String): Result<ScheduledFeed> {
    return handleError {
      ...
      val feeds =
        response?.feeds?.mapNotNull { remote ->
          val latestVersion = remote.feed_versions.firstOrNull() ?: return@mapNotNull null
          ScheduledFeed(
            feed = FeedEntity(feedId = feedID, downloadUrl = latestVersion.url),
            version =
              FeedVersion(
  ...
  override fun feeds(
    ...
    search: String,
    spec: String,
  ): Result<Collection<ScheduledFeed>> {
    return handleError {
      ...
      val feeds =
        response?.feeds?.mapNotNull { remote ->
          ...
          ScheduledFeed(
            feed = FeedEntity(feedId = feedOnestopId, downloadUrl = latestVersion.url),
            version =
              FeedVersion(
                uid = latestVersion.sha1,
  ...
  override fun feedsByCoordinates(
    ...
    return handleError {
      ...
      logger.debug(
        "Found {} feeds for coordinates ({}  {}, radius {}m)",
        feeds.size,
        lat,
        lon,
        radius,
  ...
  /**
   * Retrieve all agencies that serve a given [region] or are contained in the feed identified by
   * [feedID].
   */
  ...
  override fun agencies(apiKey: String, region: String?, feedID: String?): Result<AgencyResult> {
    return handleError {
      ...
      val agencies =
        response?.agencies?.mapNotNull { remote ->
          try {
            AgencyResultItem(
              id = remote.onestopID,
  ...
  override fun stop(apiKey: String, feedID: String, stopID: String): Result<StopResultItem> {
    return handleError {
      ...
      val result =
        response?.stops?.map { remote ->
          StopResultItem(uid = remote.onestopID, stopID = remote.stopID)
        } ?: emptyList()
  ...
  /**
   * Retrieve metadata for a specific feed. Uses concurrency control to prevent overwhelming the API
  ...
  override fun feedMetadata(apiKey: String, feedId: String): Result<FeedMetadataResult> {
    return handleError {
      ...
      val specType =
        when (feed.spec.lowercase()) {
          "gtfs" -> FeedSpecType.GTFS
          "gtfs-rt" -> FeedSpecType.GTFS_RT
          else -> {
            logger.warn(
              "Unknown spec type '{}' for feed: {}, defaulting to GTFS",
              feed.spec,
              feedId,
            )
            FeedSpecType.GTFS
  ...
  private fun <T> handleError(block: () -> Result<T>): Result<T> {
    return try {
      block()
  ...
  private fun <T : Any> get(uri: String, apiKey: String, clazz: Class<T>): T? {
  ...
  /**
   * Makes a GET request with concurrency control. Uses both a semaphore to limit concurrent
   * requests and a rate limiter to control request rate. This is suitable for parallel batch
   * operations.
   */
  private fun <T : Any> getConcurrent(uri: String, apiKey: String, clazz: Class<T>): T? {
