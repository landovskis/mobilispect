...
/**
 * A client to access the transitland API. Implementations should handle rate limiting and
 * concurrency control.
 */
interface TransitLandAPI {
  ...
  /**
   * Retrieve all feeds that match the given [search] criteria. Searches across feed names, operator
   * names, and regions.
   */
  fun feeds(
    apiKey: String,
  ...
  /**
   * Retrieve all feeds within a geographic area. Uses latitude, longitude, and radius (in meters)
   * to find feeds.
   */
  fun feedsByCoordinates(
    apiKey: String,
    lat: Double,
    lon: Double,
    radius: Int = 50000,
  ...
  /**
   * Retrieve all operators with pagination support. Returns a paginated list of operators with
   * their feeds.
   */
  fun operators(
    apiKey: String,
  ...
  /**
   * Retrieve metadata for a specific feed identified by [feedId]. Returns detailed feed information
   * including version, URLs, and authorization details.
   */
