 ...
 *
 * The writer is transactional - all variants in a chunk are saved atomically. If any error occurs,
...
class RouteVariantWriter(
  ...
  private fun recordMetrics(processed: Int, created: Int, updated: Int) {
    ...
    stepExecution
      ?.jobExecution
      ?.executionContext
      ?.putInt("variantsProcessed", cumulativeVariantsProcessed)
    stepExecution
      ?.jobExecution
      ?.executionContext
      ?.putInt("variantsCreated", cumulativeVariantsCreated)
    stepExecution
      ?.jobExecution
      ?.executionContext
      ?.putInt("variantsUpdated", cumulativeVariantsUpdated)
  }
}
