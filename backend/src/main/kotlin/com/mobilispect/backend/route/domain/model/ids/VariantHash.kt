...
/**
 * Value class for RouteVariant identifiers using SHA-256 hash. Ensures type safety and prevents ID
 * mixups across domain boundaries.
 *
 * The hash is computed from the ordered stop pattern to uniquely identify each service pattern
 * variant.
 *
...
value class VariantHash(val value: String) {
  init {
    ...
    require(value.matches(Regex("^[a-fA-F0-9]{64}$"))) { "Variant hash must be a valid hex string" }
  }
  ...
  companion object {
    fun from(value: String?): VariantHash? =
      value
        ?.takeIf { it.isNotBlank() && it.length == 64 && it.matches(Regex("^[a-fA-F0-9]{64}$")) }
        ?.let { VariantHash(it) }
  }
}
