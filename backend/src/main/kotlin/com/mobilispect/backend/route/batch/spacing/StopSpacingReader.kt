 ...
 *
 * The reader processes variants sequentially, yielding one StopSpacingInput at a time. This allows
 * the batch framework to chunk the processing and apply transaction boundaries.
 */
@Component
@StepScope
class StopSpacingReader(private val routeVariantRepository: RouteVariantRepository) :
  ...
  fun beforeStep(stepExecution: StepExecution) {
    ...
    logger.info("Initializing StopSpacingReader with {} stops from GTFS data", data.stops.size)
  ...
  override fun read(): StopSpacingInput? {
    ...
    return StopSpacingInput(variant = variant, stopsById = stopsById)
  }
}
