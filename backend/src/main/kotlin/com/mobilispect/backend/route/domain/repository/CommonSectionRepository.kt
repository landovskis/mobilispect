 ...
 *
 * Provides data access methods for geographic segments where multiple routes overlap. Common
 * sections are identified by stop patterns and support queries by stop IDs, stop count, and
 * temporal windows. All query methods support pagination for efficient data retrieval in large
 * datasets.
 *
 * A common section represents a corridor segment served by multiple route variants, useful for
 * calculating combined frequency and capacity on shared segments. Constitutional requirement: Must
 * have at least 3 stops to be meaningful.
 */
@Repository
interface CommonSectionRepository : JpaRepository<CommonSection, UUID> {
   ...
   */
  @Query(
    "SELECT cs FROM CommonSection cs WHERE cs.firstStopId = :firstStopId ORDER BY cs.stopCount DESC, cs.lastStopId ASC"
  )
   ...
   */
  @Query(
    "SELECT cs FROM CommonSection cs WHERE cs.lastStopId = :lastStopId ORDER BY cs.stopCount DESC, cs.firstStopId ASC"
  )
   ...
   */
  @Query(
    "SELECT cs FROM CommonSection cs WHERE cs.firstStopId = :firstStopId OR cs.lastStopId = :lastStopId ORDER BY cs.stopCount DESC"
  )
   ...
   */
  @Query(
    "SELECT cs FROM CommonSection cs WHERE cs.firstStopId = :firstStopId AND cs.lastStopId = :lastStopId ORDER BY cs.stopCount DESC"
  )
   ...
   */
  @Query(
    "SELECT cs FROM CommonSection cs WHERE cs.stopCount = :stopCount ORDER BY cs.firstStopId ASC, cs.lastStopId ASC"
  )
  fun findByStopCount(@Param("stopCount") stopCount: Int, pageable: Pageable): Page<CommonSection>

  /**
   * Find common sections with at least the specified number of stops.
   *
   * Useful for finding longer corridors that serve many stops. Constitutional requirement: At least
   * 3 stops, but this can query for longer sections.
   *
   ...
   */
  @Query(
    "SELECT cs FROM CommonSection cs WHERE cs.stopCount >= :minStopCount ORDER BY cs.stopCount DESC, cs.firstStopId ASC"
  )
   ...
   */
  @Query(
    "SELECT cs FROM CommonSection cs WHERE cs.stopCount <= :maxStopCount ORDER BY cs.stopCount DESC, cs.firstStopId ASC"
  )
   ...
   */
  @Query(
    "SELECT cs FROM CommonSection cs WHERE cs.stopCount >= :minStopCount AND cs.stopCount <= :maxStopCount ORDER BY cs.stopCount DESC, cs.firstStopId ASC"
  )
   ...
   */
  @Query(
    "SELECT cs FROM CommonSection cs WHERE cs.createdAt >= :after AND cs.createdAt <= :before ORDER BY cs.createdAt DESC"
  )
  ...
  fun findByUpdatedAtAfter(@Param("since") since: Instant, pageable: Pageable): Page<CommonSection>
   ...
   */
  @Query(
    "SELECT CASE WHEN COUNT(cs) > 0 THEN true ELSE false END FROM CommonSection cs WHERE cs.stopPattern = :stopPattern"
  )
   ...
   */
  @Query(
    "SELECT CASE WHEN COUNT(cs) > 0 THEN true ELSE false END FROM CommonSection cs WHERE cs.firstStopId = :firstStopId AND cs.lastStopId = :lastStopId"
  )
   ...
   */
  @Query("SELECT COALESCE(MAX(cs.stopCount), 0) FROM CommonSection cs") fun getMaxStopCount(): Int
