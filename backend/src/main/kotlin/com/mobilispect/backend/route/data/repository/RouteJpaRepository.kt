...
interface RouteJpaRepository : JpaRepository<RouteEntity, String> {
     ...
     */
    @Query("SELECT r FROM RouteEntity r WHERE r.agencyOnestopId = :agencyId ORDER BY r.shortName ASC, r.longName ASC")
    fun findByAgencyId(
     ...
     */
    @Query("SELECT r FROM RouteEntity r WHERE r.agencyOnestopId = :agencyId AND r.active = true ORDER BY r.shortName ASC, r.longName ASC")
     ...
     */
    @Query("SELECT r FROM RouteEntity r WHERE r.agencyOnestopId = :agencyId AND r.routeType = :routeType ORDER BY r.shortName ASC, r.longName ASC")
     ...
     */
    @Query("SELECT r FROM RouteEntity r WHERE r.agencyOnestopId = :agencyId AND r.routeType = :routeType AND r.active = true ORDER BY r.shortName ASC, r.longName ASC")
     ...
     */
    @Query("SELECT r FROM RouteEntity r WHERE r.agencyOnestopId = :agencyId AND r.gtfsRouteId = :gtfsRouteId")
     ...
     */
    @Query("SELECT r FROM RouteEntity r WHERE r.active = :active ORDER BY r.shortName ASC, r.longName ASC")
    fun findByActive(
     ...
     */
    @Query("SELECT r FROM RouteEntity r WHERE r.routeType = :routeType ORDER BY r.shortName ASC, r.longName ASC")
    fun findByRouteType(
        @Param("routeType") routeType: String,
        pageable: Pageable
    ): Page<RouteEntity>
     ...
     */
    @Query("SELECT COUNT(r) FROM RouteEntity r WHERE r.agencyOnestopId = :agencyId AND r.active = true")
    fun countActiveByAgencyId(@Param("agencyId") agencyId: String): Long

    /**
     * Count routes of a specific type for an agency.
     */
    @Query("SELECT COUNT(r) FROM RouteEntity r WHERE r.agencyOnestopId = :agencyId AND r.routeType = :routeType")
     ...
     */
    @Query("SELECT CASE WHEN COUNT(r) > 0 THEN true ELSE false END FROM RouteEntity r WHERE r.agencyOnestopId = :agencyId AND r.gtfsRouteId = :gtfsRouteId")
