...
interface RouteJpaRepository : JpaRepository<RouteEntity, String> {
  ...
  @Query(
    "SELECT r FROM RouteEntity r WHERE r.agencyOnestopId = :agencyId AND r.gtfsRouteId = :gtfsRouteId"
  )
  ...
  @Query(
    "SELECT CASE WHEN COUNT(r) > 0 THEN true ELSE false END FROM RouteEntity r WHERE r.agencyOnestopId = :agencyId AND r.gtfsRouteId = :gtfsRouteId"
  )
