 ...
 * providing accurate distance measurements for geographic coordinates.
 */
@Component
@StepScope
class StopSpacingProcessor : ItemProcessor<StopSpacingInput, StopSpacingBatch> {
     ...
     * Returns list of StopSpacing entities, or empty list if cannot be calculated.
     */
    private fun calculateStopSpacings(
        ...
        if (stopIds.size < 2) {
            logger.debug(
                "Variant {} has insufficient stops ({} stops)",
                variant.id.value,
                stopIds.size
            )
            return emptyList()
        ...
        for (i in 0 until stopIds.size - 1) {
            ...
            if (fromStop?.latitude != null && fromStop.longitude != null &&
