...
class RouteRepositoryImpl(
  ...
  override fun findById(routeId: RouteId): Route? =
    jpaRepository.findById(routeId.value).map { mapper.toDomain(it) }.orElse(null)
  ...
  override fun findByAgencyId(agencyId: AgencyId, pageable: Pageable): Page<Route> {
    ...
    return jpaRepository.findByAgencyId(agencyId.value, pageable).map { mapper.toDomain(it) }
  }
  ...
  override fun findByAgencyIdAndActive(agencyId: AgencyId, pageable: Pageable): Page<Route> {
    ...
    agencyQueryApi.findAgencyById(agencyId)
      ?: throw IllegalArgumentException("Agency not found: $agencyId")
    return jpaRepository.findByAgencyIdAndActive(agencyId.value, pageable).map {
      mapper.toDomain(it)
    }
  }

  override fun findByAgencyIdAndRouteType(
    agencyId: AgencyId,
    routeType: RouteType,
    pageable: Pageable,
  ): Page<Route> {
    ...
    agencyQueryApi.findAgencyById(agencyId)
      ?: throw IllegalArgumentException("Agency not found: $agencyId")
    return jpaRepository.findByAgencyIdAndRouteType(agencyId.value, routeType.value, pageable).map {
      mapper.toDomain(it)
    }
  }

  override fun findByAgencyIdAndRouteTypeAndActive(
    agencyId: AgencyId,
    routeType: RouteType,
    pageable: Pageable,
  ): Page<Route> {
    ...
    return jpaRepository
      .findByAgencyIdAndRouteTypeAndActive(agencyId.value, routeType.value, pageable)
      .map { mapper.toDomain(it) }
  }

  override fun findByAgencyIdAndGtfsRouteId(agencyId: AgencyId, gtfsRouteId: String): Route? =
    jpaRepository.findByAgencyIdAndGtfsRouteId(agencyId.value, gtfsRouteId)?.let {
      mapper.toDomain(it)
    }
  ...
  override fun findByRouteType(routeType: RouteType, pageable: Pageable): Page<Route> =
    jpaRepository.findByRouteType(routeType.value, pageable).map { mapper.toDomain(it) }
  ...
  override fun findAll(): List<Route> = jpaRepository.findAll().map { mapper.toDomain(it) }
}
