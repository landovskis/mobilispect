 ...
 *
 * Provides data access for route variant entities using plain String IDs. Used by
 * RouteVariantRepository implementation to persist and retrieve domain models.
 */
interface RouteVariantJpaRepository : JpaRepository<RouteVariantEntity, String> {
  ...
  @Query(
    "SELECT rv FROM RouteVariantEntity rv WHERE rv.route.id = :routeId ORDER BY rv.stopCount ASC, rv.id ASC"
  )
  fun findByRouteId(@Param("routeId") routeId: String): List<RouteVariantEntity>

  /** Find all variants for a specific route. */
  @Query(
    "SELECT rv FROM RouteVariantEntity rv WHERE rv.route = :route ORDER BY rv.stopCount ASC, rv.id ASC"
  )
  fun findByRoute(@Param("route") route: RouteEntity, pageable: Pageable): Page<RouteVariantEntity>

  /** Find all active variants for a specific route. */
  @Query(
    "SELECT rv FROM RouteVariantEntity rv WHERE rv.route = :route AND rv.active = true ORDER BY rv.stopCount ASC, rv.id ASC"
  )
  ...
  @Query(
    "SELECT rv FROM RouteVariantEntity rv WHERE rv.route = :route AND rv.directionId = :directionId ORDER BY rv.stopCount ASC, rv.id ASC"
  )
  ...
  @Query(
    "SELECT rv FROM RouteVariantEntity rv WHERE rv.route = :route AND rv.directionId = :directionId AND rv.active = true ORDER BY rv.stopCount ASC, rv.id ASC"
  )
  ...
  @Query(
    "SELECT rv FROM RouteVariantEntity rv JOIN rv.route r WHERE rv.stopCount = :stopCount ORDER BY r.id ASC, rv.id ASC"
  )
  fun findByStopCount(
  ...
  @Query(
    "SELECT rv FROM RouteVariantEntity rv JOIN rv.route r WHERE rv.active = :active ORDER BY r.id ASC, rv.stopCount ASC"
  )
  fun findByActive(@Param("active") active: Boolean, pageable: Pageable): Page<RouteVariantEntity>

  /** Find variants first observed in a time window. */
  @Query(
    "SELECT rv FROM RouteVariantEntity rv WHERE rv.firstSeen >= :after AND rv.firstSeen <= :before ORDER BY rv.firstSeen DESC"
  )
  ...
  @Query(
    "SELECT rv FROM RouteVariantEntity rv WHERE rv.lastSeen >= :since ORDER BY rv.lastSeen DESC"
  )
  ...
  @Query(
    "SELECT rv FROM RouteVariantEntity rv JOIN rv.route r WHERE rv.firstStopId = :firstStopId AND rv.lastStopId = :lastStopId ORDER BY r.id ASC"
  )
  ...
  @Query(
    "SELECT COUNT(rv) FROM RouteVariantEntity rv WHERE rv.route = :route AND rv.directionId = :directionId"
  )
  ...
  @Query(
    "SELECT CASE WHEN COUNT(rv) > 0 THEN true ELSE false END FROM RouteVariantEntity rv WHERE rv.route.id = :routeId AND rv.firstStopId = :firstStopId AND rv.lastStopId = :lastStopId"
  )
