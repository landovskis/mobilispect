 ...
 *
 * Provides data access methods for stop spacing data between consecutive stops on route variants.
 */
@Repository
interface StopSpacingRepository : JpaRepository<StopSpacing, UUID> {
   ...
   */
  @Query(
    "SELECT s FROM StopSpacing s WHERE s.variantId = :variantId AND s.fromStopId = :fromStopId AND s.toStopId = :toStopId"
  )
   ...
   */
  @Query(
    "SELECT CASE WHEN COUNT(s) > 0 THEN true ELSE false END FROM StopSpacing s WHERE s.variantId = :variantId"
  )
