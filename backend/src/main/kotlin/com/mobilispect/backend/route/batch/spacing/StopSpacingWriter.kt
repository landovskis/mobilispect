 ...
 *
 * The writer is transactional - all spacing records in a chunk are saved atomically.
...
class StopSpacingWriter(
    ...
    private fun recordMetrics(variantsProcessed: Int, spacingsCreated: Int) {
        ...
        stepExecution?.jobExecution?.executionContext?.putInt("stopSpacingVariantsProcessed", cumulativeVariantsProcessed)
        stepExecution?.jobExecution?.executionContext?.putInt("stopSpacingRecordsCreated", cumulativeSpacingsCreated)
    }
}
