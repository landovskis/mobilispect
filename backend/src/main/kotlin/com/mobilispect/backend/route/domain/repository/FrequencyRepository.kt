 ...
 *
 * Provides data access methods for service frequency data, including queries by route variant,
 * service date, time period, and headway metrics. All query methods support pagination for
 * efficient data retrieval in large datasets.
 *
 * Frequency records track how often a route variant operates during different times of day and days
 * of week, measured as headway (time between consecutive vehicles).
 */
@Repository
interface FrequencyRepository : JpaRepository<Frequency, UUID> {
   ...
   */
  @Query(
    "SELECT f FROM Frequency f WHERE f.variantId = :variantId ORDER BY f.serviceDate DESC, f.timePeriod ASC"
  )
  fun findByVariant(@Param("variantId") variantId: String, pageable: Pageable): Page<Frequency>
   ...
   */
  @Query(
    "SELECT f FROM Frequency f WHERE f.variantId = :variantId AND f.serviceDate = :serviceDate ORDER BY f.timePeriod ASC"
  )
   ...
   *
   * This is the primary query for retrieving specific headway data for a route variant at a
   ...
   */
  @Query(
    "SELECT f FROM Frequency f WHERE f.variantId = :variantId AND f.serviceDate = :serviceDate AND f.timePeriod = :timePeriod"
  )
   ...
   */
  @Query(
    "SELECT f FROM Frequency f WHERE f.serviceDate >= :startDate AND f.serviceDate <= :endDate ORDER BY f.serviceDate DESC, f.timePeriod ASC"
  )
   ...
   */
  @Query(
    "SELECT f FROM Frequency f WHERE f.variantId = :variantId AND f.serviceDate >= :startDate AND f.serviceDate <= :endDate ORDER BY f.serviceDate DESC, f.timePeriod ASC"
  )
   ...
   */
  @Query(
    "SELECT f FROM Frequency f WHERE f.timePeriod = :timePeriod ORDER BY f.serviceDate DESC, f.variantId ASC"
  )
   ...
   */
  @Query(
    "SELECT f FROM Frequency f WHERE f.variantId = :variantId AND f.isIrregular = true ORDER BY f.serviceDate DESC"
  )
   ...
   */
  @Query(
    "SELECT f FROM Frequency f WHERE f.averageHeadway IS NOT NULL AND f.averageHeadway < :maxHeadway ORDER BY f.averageHeadway ASC"
  )
   ...
   */
  @Query(
    "SELECT f FROM Frequency f WHERE f.variantId = :variantId ORDER BY f.serviceDate DESC LIMIT :limit"
  )
   ...
   */
  @Query(
    "SELECT COUNT(f) FROM Frequency f WHERE f.serviceDate >= :startDate AND f.serviceDate <= :endDate"
  )
   ...
   */
  @Query(
    "SELECT CASE WHEN COUNT(f) > 0 THEN true ELSE false END FROM Frequency f WHERE f.variantId = :variantId AND f.serviceDate = :serviceDate AND f.timePeriod = :timePeriod"
  )
