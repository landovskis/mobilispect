 ...
 *
 * The writer is transactional - all frequencies in a chunk are saved atomically. If any error
...
class FrequencyWriter(
  ...
  override fun write(chunk: Chunk<out FrequencyBatch>) {
    ...
    processedVariants.forEach { (variantId, serviceDate) ->
      eventPublisher.publishEvent(
        FrequencyCalculationCompleted(variantId = variantId, serviceDate = serviceDate)
      )
    }
  ...
  private fun recordMetrics(processed: Int, created: Int, updated: Int) {
    ...
    stepExecution
      ?.jobExecution
      ?.executionContext
      ?.putInt("frequenciesProcessed", cumulativeFrequenciesProcessed)
    stepExecution
      ?.jobExecution
      ?.executionContext
      ?.putInt("frequenciesCreated", cumulativeFrequenciesCreated)
    stepExecution
      ?.jobExecution
      ?.executionContext
      ?.putInt("frequenciesUpdated", cumulativeFrequenciesUpdated)
  }
}
