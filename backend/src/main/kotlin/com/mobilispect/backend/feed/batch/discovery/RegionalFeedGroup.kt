 ...
 *
 * This data class captures location information extracted from operator data, used to associate
 * feeds with their geographic regions.
 *
 ...
 *
 * This is the intermediate output from processing Transit.land operators, providing a map of feeds
 * to their associated geographic regions for downstream batch processing.
 *
 * @property feedToRegionMap Map of feed onestop ID to RegionMetadata
 */
data class FeedRegionMap(val feedToRegionMap: Map<FeedId, RegionMetadata>) {
  ...
  fun groupByRegion(): Map<String, Set<FeedId>> {
    return feedToRegionMap.entries.groupBy({ it.value.regionOnestopId }, { it.key }).mapValues {
      it.value.toSet()
    }
  }
