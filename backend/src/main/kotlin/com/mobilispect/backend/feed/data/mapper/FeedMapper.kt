 ...
 *
 * Per ADR 0009 FK-only pattern, regions are managed as Set<RegionId> in domain and
 * MutableSet<MetropolitanRegionEntity> in entity.
 */
@Component
class FeedMapper {

  /**
   * Converts data entity to domain model. Extracts region IDs from the many-to-many regions
   * collection.
   */
   ...
   *
   * Note: regionIds are NOT mapped here - they must be managed separately through the feed_regions
   * junction table to maintain FK-only pattern. The repository implementation is responsible for
   * populating the regions collection.
   *
