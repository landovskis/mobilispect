 ...
 *
 * Converts between the RegionId value class and its underlying String representation for database
...
class RegionIdConverter : AttributeConverter<RegionId, String> {
  ...
  override fun convertToDatabaseColumn(attribute: RegionId?): String? = attribute?.value

  override fun convertToEntityAttribute(dbData: String?): RegionId? = dbData?.let { RegionId(it) }
}
