 ...
 *
 * Task T050: Create ImportHistoryService for historical data queries Per User Story 3:
 * Administrators can view comprehensive history of feed imports
 *
 ...
 */
@Service
class ImportHistoryService(private val feedImportRepository: FeedImportRepository) {
  ...
  fun getImportHistory(
    ...
    return when {
      ...
      status != null -> feedImportRepository.findAllByStatusIn(listOf(status), pageable)
      triggerType != null ->
  ...
  fun getLastImportForFeed(feedOnestopId: String): FeedImport? {
    return feedImportRepository
      .findAllByFeedIdOrderByStartedAtDesc(
        feedOnestopId,
