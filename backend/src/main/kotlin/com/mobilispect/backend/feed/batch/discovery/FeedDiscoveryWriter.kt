 ...
 *
 * The writer is transactional and will rollback all changes if any error occurs during the write
 * operation. It handles both new feeds (insert) and existing feeds (update) appropriately.
 *
 * Duplicate regions are deduplicated automatically - if multiple feeds belong to the same region,
 * the region is only created once.
 */
@Component
@StepScope
class FeedDiscoveryWriter(
  ...
  override fun write(chunk: Chunk<out FeedDiscoveryBatch>) {
    ...
    for ((batchIndex, batch) in batches.withIndex()) {
      ...
      val uniqueRegions = batch.results.map { it.region }.distinctBy { it.regionOnestopId }
      ...
      for (regionMetadata in uniqueRegions) {
        val existingRegion =
          regionRepository
            .findByRegionOnestopId(RegionId(regionMetadata.regionOnestopId))
            .orElse(null)
          ...
          regionRepository
            ...
            .orElseThrow {
              IllegalStateException(
                "Region not found after event handling: ${regionMetadata.regionOnestopId}"
              )
            }
      ...
      for (result in batch.results) {
        ...
        val existingFeed = feedRepository.findByFeedOnestopId(feedId.value).orElse(null)
        ...
        val feedEntity =
          if (existingFeed != null) {
            ...
            logger.info(
              "      ✓ Creating feed: {} ({})",
              result.name,
              result.region.operatorName ?: "unknown operator",
            )
            feedsCreated++
            val regionEntity =
              regionEntities[result.region.regionOnestopId]
            ...
            FeedEntity(
              ...
              .apply { regions.add(regionEntity) }
          }
      ...
      logger.info(
        "    ✓ Batch complete: {} feeds saved ({} new)",
        batch.results.size,
        batchNewFeeds,
      )
    }
 ...
 *
 * This data class captures metrics about the write operation for monitoring and reporting purposes.
 */
