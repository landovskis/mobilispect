 ...
 *
 * Task T042: Create FeedVersionService for SHA1-based change detection Per FR-013: System MUST
 * check feeds using SHA-1 content hash comparison
 *
 * This service:
 ...
 *
 * SHA-1 hashes are computed from complete feed archives to detect any changes in the feed data,
 * ensuring accurate change detection per FR-016.
 */
@Service
class FeedVersionService(
  ...
  fun hasUpdate(feedOnestopId: FeedId): Boolean {
    val feed = feedRepository.findByFeedOnestopId(feedOnestopId.value).orElse(null)
    if (feed == null) {
