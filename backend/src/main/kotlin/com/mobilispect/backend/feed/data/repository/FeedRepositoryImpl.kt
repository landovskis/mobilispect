 ...
 * Manages many-to-many relationship with regions through junction table per ADR 0009 FK-only pattern.
 */
...
class FeedRepositoryImpl(
    ...
    override fun findById(feedId: FeedId): Feed? =
        jpaRepository.findByFeedOnestopId(feedId.value)
            ?.let { mapper.toDomain(it) }

    override fun findByRegionId(regionId: RegionId): List<Feed> =
        jpaRepository.findByRegionId(regionId.value)
            .map { mapper.toDomain(it) }

    override fun findByRegionIdAndStatusIn(regionId: RegionId, statuses: Collection<FeedStatus>): List<Feed> =
        jpaRepository.findByRegionIdAndStatusIn(regionId.value, statuses)
            .map { mapper.toDomain(it) }

    override fun findByRegionIdAndSpecTypeIn(regionId: RegionId, specTypes: Collection<FeedSpecType>): List<Feed> =
        jpaRepository.findByRegionIdAndSpecTypeIn(regionId.value, specTypes)
            .map { mapper.toDomain(it) }
