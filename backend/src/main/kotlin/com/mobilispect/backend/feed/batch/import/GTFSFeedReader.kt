 ...
 *
 * This service handles GTFS feed imports for the feed management system, downloading and processing
 * feeds.
 */
@Service
@StepScope
class GTFSFeedReader(
  ...
  fun importFeedById(feedId: FeedId): Result<GTFSData> {
    ...
    val archive =
      doStep(feedId, "download") { downloadFeed(feed) }.getOrNull()
        ?: return Result.failure(IllegalStateException("Download failed"))

    val extractedDir =
      doStep(feedId, "extract") { extractFeed(archive) }.getOrNull()
        ?: return Result.failure(IllegalStateException("Extraction failed"))

    doStep(feedId, "validate") { validateGtfsFiles(extractedDir) }.getOrNull()
  ...
  override fun read(): GTFSData? {
    ...
    val feedId =
      FeedId.Companion.from(feedOnestopId)
   ...
   *
   * Uses the Conveyal gtfs-lib (6.2.0), which is the successor to OneBusAway. Provides better
   * performance for large feeds with disk-backed storage via MapDB.
   *
  ...
  fun parse(feedPath: Path): Result<GTFSData> = runCatching {
    ...
    feed.use { feed ->
      ...
      val trips =
        feed.trips.values.map { trip ->
          val stopTimes =
            feed.getOrderedStopTimesForTrip(trip.trip_id).map { stopTime ->
              GTFSStopTime(
