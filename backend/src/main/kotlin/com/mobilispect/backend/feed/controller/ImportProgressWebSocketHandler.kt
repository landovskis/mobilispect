 ...
 *
 * This controller handles WebSocket connections for real-time import progress updates. Clients
 * subscribe to /topic/import/progress/{importId} to receive updates.
 *
 * WebSocket Topics:
 ...
 *
 * This implementation delegates to the existing ProgressWebSocketController in the websocket
 * package, which manages the actual WebSocket connections and message routing. This facade provides
 * the feed domain with a clean interface to WebSocket handling while maintaining separation of
 * concerns.
 *
 * Usage from frontend:
 * ```typescript
...
class ImportProgressWebSocketHandler(
  private val progressWebSocketController:
    com.mobilispect.backend.websocket.ProgressWebSocketController
) {

  /**
   * Handle progress subscription requests from clients. Clients send a message to
   * /app/import/subscribe with importId to start receiving updates.
   */
