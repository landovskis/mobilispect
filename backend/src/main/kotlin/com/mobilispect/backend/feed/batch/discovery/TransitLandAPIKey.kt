 ...
 *
 * This value class provides type safety for API keys, preventing accidental mixing of API keys with
 * other string values and making the API more explicit.
 *
 * Value classes have zero runtime overhead - they are represented as the underlying String type at
 * runtime with no wrapper object allocation.
 *
...
value class TransitLandAPIKey(val value: String) {
  ...
  companion object {
    /**
     * Creates a TransitLandAPIKey from a nullable string. Returns null if the input is null or
     * blank.
     */
