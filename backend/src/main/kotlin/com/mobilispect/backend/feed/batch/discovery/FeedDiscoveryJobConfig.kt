 ...
 *
 * The job is designed to be run periodically (e.g., daily) to keep the feed database synchronized
 * with Transit.land.
 */
@Configuration
class SimplifiedFeedDiscoveryJobConfig(
   ...
   *
   * This job processes operators in batches, fetching metadata and writing to the database all in
   * one step for simplicity.
   */
  @Bean
  fun simplifiedFeedDiscoveryJob(
    @Qualifier("simplifiedDiscoveryStep") simplifiedDiscoveryStep: Step
  ): Job =
    JobBuilder("simplifiedFeedDiscoveryJob", jobRepository).start(simplifiedDiscoveryStep).build()
  ...
  fun simplifiedDiscoveryStep(
    @Qualifier("combinedFeedDiscoveryReader") reader: ItemReader<FeedDiscoveryInput>
  ): Step =
    StepBuilder("simplifiedDiscoveryStep", jobRepository)
  ...
  override fun afterJob(jobExecution: org.springframework.batch.core.job.JobExecution) {
    val duration =
      if (jobExecution.endTime != null && jobExecution.startTime != null) {
        java.time.Duration.between(jobExecution.startTime, jobExecution.endTime).toMillis()
      } else {
        0L
      }
