 ...
 *
 * Provides observability and monitoring for each step in the feed import process
 ...
 */
@Component
class FeedImportStepExecutionListener(
    ...
    override fun beforeStep(stepExecution: StepExecution) {
        ...
        logger.info(
            "Starting feed import step: {} for feed: {}",
            stepName,
     ...
     *
     * The feedId can be stored in either the job execution context (set by GTFSFeedReader)
     * or passed as a job parameter (feedOnestopId).
     */
    private fun extractFeedId(stepExecution: StepExecution): FeedId? {
        ...
        return when {
            ...
            feedOnestopId != null -> FeedId.from(feedOnestopId)
                ...
                logger.warn(
                    "Could not extract feedId from step execution: {}",
                    stepExecution.stepName
                )
                null
            }
