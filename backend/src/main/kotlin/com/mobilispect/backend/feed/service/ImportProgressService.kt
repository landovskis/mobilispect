 ...
 *
 * This service provides the feed domain with a clean interface to progress tracking while
 * maintaining separation of concerns. Progress tracking is now handled by the
 * FeedImportProgressEventListener which listens to feed import events and publishes WebSocket
 * updates automatically.
 *
 * This facade delegates to the event listener for querying current progress state, while progress
 * updates are driven by publishing FeedImport domain events.
 */
@Service
class ImportProgressService(private val progressEventListener: FeedImportProgressEventListener) {
