 ...
 *
 * This component listens for application startup and automatically discovers feeds for any regions
 * that have auto-update enabled but have zero feeds. This ensures that newly added regions or fresh
...
class FeedDiscoveryStartup(
  ...
  fun runInitialDiscovery() {
    ...
    runBlocking {
      ...
      if (totalFeeds > 0) {
        logger.info(
          "Feed database already populated with {} feeds. Skipping initial discovery.",
          totalFeeds,
        )
        return@runBlocking
      }
      ...
      runCatching { feedDiscoveryBatchService.discoverAll(FeedSpecType.GTFS) }
        .onSuccess { result ->
          logger.info(
          ...
          if (result.errors.isNotEmpty()) {
            ...
            result.errors.take(10).forEach { error -> logger.warn("  - {}", error) }
        ...
        .onFailure { throwable ->
          logger.error("Global {} discovery failed", FeedSpecType.GTFS, throwable)
        }

      val finalCount = feedRepository.count()
      logger.info(
        "Initial global feed discovery completed. Total feeds in database: {}",
        finalCount,
      )
    }
  }
