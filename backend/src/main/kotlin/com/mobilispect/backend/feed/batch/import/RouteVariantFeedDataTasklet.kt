...
import com.mobilispect.backend.route.handler.RouteVariantFeedDataHandler
...
@Component
@StepScope
class RouteVariantFeedDataTasklet(
  private val routeVariantFeedDataHandler: RouteVariantFeedDataHandler
) : Tasklet {

  private val logger = LoggerFactory.getLogger(RouteVariantFeedDataTasklet::class.java)
  ...
  override fun execute(contribution: StepContribution, chunkContext: ChunkContext): RepeatStatus {
    ...
    val result = routeVariantFeedDataHandler.handle(feedId, bundle, importContext)

    when (result) {
      is ImportResult.Success -> {
        logger.info(
          "Route variant handler completed for feed {}: {} records processed",
          feedId.value,
      ...
      is ImportResult.PartialSuccess -> {
        logger.warn(
          "Route variant handler partially completed for feed {}: {} succeeded, {} failed",
          feedId.value,
          result.recordsProcessed,
          result.errors.size,
        )
      ...
      is ImportResult.Failure -> {
        logger.error(
          "Route variant handler failed for feed {}: {}",
          feedId.value,
          result.error.message,
        )
        throw IllegalStateException(result.error.message ?: "Route variant handler failed")
      }
    }
