 ...
 *
 * Uses sealed classes to encode the reader's phase in the type system, ensuring compile-time safety
 * for state transitions and data availability.
 *
 * State transitions:
...
sealed class ReaderState {
  ...
  /**
   * State after all operators have been read. Contains the complete feed-region mapping and
   * prepared batches.
   */
  data class OperatorsRead(val feedRegionMap: FeedRegionMap, val batches: List<List<FeedId>>) :
    ReaderState()

  /**
   * State after metadata has been fetched for all feeds. Ready to begin yielding batches to Spring
   * Batch.
   */
  ...
  /** State while yielding batches to Spring Batch. Tracks current position in the batch list. */
  data class Yielding(
    val feedRegionMap: FeedRegionMap,
    val feedMetadataMap: FeedMetadataMap,
    val batches: List<List<FeedId>>,
 ...
 *
 * This reader orchestrates a multi-phase read process ensuring all operators are read before
 * metadata fetching begins:
 *
 * Phase 1 (Read): Read ALL operators and build complete FeedRegionMap Phase 2 (Fetch): Fetch
 * metadata for ALL discovered feeds Phase 3 (Return): Return batches of FeedDiscoveryInput for
 * processing
 *
 * This ensures the reading phase completes entirely before processing begins, which is critical for
 * efficient API usage and data consistency.
 *
 * Rate limiting and concurrency control are centralized in TransitLandClient.
 *
 * Uses a type-safe state machine (sealed classes) to track progress through phases, ensuring
...
class FeedDiscoveryReader(
    ...
    return when (val currentState = state) {
      ...
      is ReaderState.Yielding -> {
        ...
        if (currentState.nextBatchIndex >= currentState.batches.size) {
          ...
          FeedDiscoveryInput(feedRegionMap = batchRegionMap, feedMetadataMap = batchMetadataMap)
        }
      }
  ...
  private fun readAllOperators(): FeedRegionMap {
    ...
    logger.info(
      "✓ Phase 1 complete: Discovered {} feeds across {} operator pages",
      allMappings.size,
      batchCount,
    )
  ...
  private fun transformOperatorsToFeedRegionMap(
    ...
    for (operator in operators) {
      val allFeedIds =
        operator.feeds
          ...
          ?.mapNotNull { it.onestop_id } ?: emptyList()
      ...
      val place =
        firstAgency?.places?.firstOrNull { it.city_name != null }
          ?: firstAgency?.places?.firstOrNull()

      val regionParts =
        if (place != null) {
          listOfNotNull(place.city_name, place.adm1_name, place.adm0_name)
        } else {
          logger.warn(
      ...
      val isMontreal =
        regionParts.any {
          it.contains("Montréal", ignoreCase = true) || it.contains("Montreal", ignoreCase = true)
        }
      if (isMontreal) {
        logger.info(
          "      🍁 MONTREAL: {} → {} ({} feeds)",
      ...
      val sanitizedParts =
        regionParts
          .map {
            it.lowercase().replace(Regex("[^a-z0-9-]"), "-").replace(Regex("-+"), "-").trim('-')
          }
  ...
  /**
   * Phase 2: Fetch metadata for all discovered feed IDs. Uses the centralized TransitLandClient
   * which handles rate limiting and concurrency.
   */
  private fun fetchAllMetadata(feedRegionMap: FeedRegionMap): FeedMetadataMap {
    ...
    logger.info(
      "Phase 2: Fetching metadata for {} feeds in {} batches",
      feedIds.size,
      feedIdChunks.size,
    )
    ...
    if (failedCount > 0) {
      logger.info(
        "✓ Phase 2 complete: Fetched metadata for {}/{} feeds ({} failed)",
        allMetadata.size,
        feedIds.size,
        failedCount,
      )
  ...
  /**
   * Fetches metadata for a single feed using the centralized TransitLandClient. Rate limiting and
   * concurrency control are handled by the client.
   */
