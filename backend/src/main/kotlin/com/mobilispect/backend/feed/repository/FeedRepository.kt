...
interface FeedRepository : JpaRepository<FeedEntity, String> {
   ...
   *
   * This method is needed because Hibernate's findById doesn't properly convert the FeedId value
   * class for ID lookups. Use this instead of findById(FeedId).
   */
  ...
  fun findAllByRegionRegionOnestopId(
    @Param("regionOnestopId") regionOnestopId: RegionId
  ): List<FeedEntity>

  @Query(
    "SELECT COUNT(f) FROM FeedEntity f JOIN f.regions r WHERE r.regionOnestopId = :regionOnestopId"
  )
  ...
  @Query(
    "SELECT f FROM FeedEntity f JOIN f.regions r WHERE r.regionOnestopId = :regionOnestopId AND f.specType IN :specTypes"
  )
