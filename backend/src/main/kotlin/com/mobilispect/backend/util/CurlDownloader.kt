...
/**
 * A [Downloader] that uses curl command-line tool. This is used as a fallback when WebClient fails
 * or for compatibility with servers that have strict bot protection.
 */
...
internal class CurlDownloader : Downloader {
  ...
  override fun download(request: DownloadRequest): Result<Path> {
    ...
    return try {
      ...
      logger.info(
        "Download complete via curl. File size: {} bytes. Path: {}",
        dest.fileSize(),
        dest,
      )
      Result.success(dest)
