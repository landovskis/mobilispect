...
class AgencyProcessor(
  ...
  private fun loadAgencyLookup(): Map<GTFSAgencyId, String> {
    val apiKey =
      credentialsRepository.get() ?: throw IllegalStateException("Missing Transitland API key")
    val feedId =
      feedOnestopId.ifBlank {
        throw IllegalArgumentException("feedOnestopId job parameter is required")
      }
    val result =
      transitLandAPI.agencies(apiKey = apiKey, feedID = feedId).getOrElse {
        throw IllegalStateException("Transitland agency lookup failed for feed=$feedId", it)
      }
