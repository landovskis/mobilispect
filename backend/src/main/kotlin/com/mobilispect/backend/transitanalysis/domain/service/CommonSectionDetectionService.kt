...
interface CommonSectionDetectionService {
    fun detectCommonSections(variants: List<RouteVariant>): Pair<List<CommonSection>, List<CommonSectionVariant>>
}

@Service
class CommonSectionDetectionServiceImpl : CommonSectionDetectionService {
    override fun detectCommonSections(variants: List<RouteVariant>): Pair<List<CommonSection>, List<CommonSectionVariant>> {
        ...
        variants.forEachIndexed { index, a ->
            ...
            variants.drop(index + 1).forEach { b ->
                ...
                val common = longestCommonSubsequence(stopsA, stopsB)
                    .filter { it.size >= 3 }
                common.forEach { seq ->
