...
import com.mobilispect.backend.transitanalysis.data.mapper.RouteVariantMapper
...
class RouteVariantRepositoryImpl(
    ...
    override fun findByRouteId(routeId: RouteId): List<RouteVariant> =
        jpaRepository.findByRouteId(routeId.value)
            .map { mapper.toDomain(it) }

    override fun findByRouteId(routeId: RouteId, pageable: Pageable): Page<RouteVariant> {
        val routeEntity = routeJpaRepository.findById(routeId.value)
            .orElseThrow { IllegalArgumentException("Route not found: $routeId") }
        return jpaRepository.findByRoute(routeEntity, pageable)
            .map { mapper.toDomain(it) }
    }
    ...
    override fun findByRouteIdAndActive(routeId: RouteId, pageable: Pageable): Page<RouteVariant> {
        val routeEntity = routeJpaRepository.findById(routeId.value)
            .orElseThrow { IllegalArgumentException("Route not found: $routeId") }
        return jpaRepository.findByRouteAndActive(routeEntity, pageable)
            .map { mapper.toDomain(it) }
    }

    override fun findByRouteIdAndDirectionId(routeId: RouteId, directionId: Int, pageable: Pageable): Page<RouteVariant> {
        val routeEntity = routeJpaRepository.findById(routeId.value)
            .orElseThrow { IllegalArgumentException("Route not found: $routeId") }
        return jpaRepository.findByRouteAndDirectionId(routeEntity, directionId, pageable)
            .map { mapper.toDomain(it) }
    }

    override fun findByRouteIdAndDirectionIdAndActive(routeId: RouteId, directionId: Int, pageable: Pageable): Page<RouteVariant> {
        val routeEntity = routeJpaRepository.findById(routeId.value)
            .orElseThrow { IllegalArgumentException("Route not found: $routeId") }
        return jpaRepository.findByRouteAndDirectionIdAndActive(routeEntity, directionId, pageable)
            .map { mapper.toDomain(it) }
    }

    override fun findByStopCount(stopCount: Int, pageable: Pageable): Page<RouteVariant> =
        jpaRepository.findByStopCount(stopCount, pageable)
            .map { mapper.toDomain(it) }
    ...
    override fun findByFirstSeenBetween(after: Instant, before: Instant, pageable: Pageable): Page<RouteVariant> =
        jpaRepository.findByFirstSeenBetween(after, before, pageable)
            .map { mapper.toDomain(it) }

    override fun findByLastSeenAfter(since: Instant, pageable: Pageable): Page<RouteVariant> =
        jpaRepository.findByLastSeenAfter(since, pageable)
            .map { mapper.toDomain(it) }

    override fun findByFirstStopIdAndLastStopId(firstStopId: String, lastStopId: String, pageable: Pageable): Page<RouteVariant> =
        jpaRepository.findByFirstStopIdAndLastStopId(firstStopId, lastStopId, pageable)
            .map { mapper.toDomain(it) }
    ...
    override fun existsByRouteIdAndFirstStopIdAndLastStopId(routeId: RouteId, firstStopId: String, lastStopId: String): Boolean =
    ...
    override fun save(variant: RouteVariant): RouteVariant {
        val routeEntity = routeJpaRepository.findById(variant.routeId.value)
            .orElseThrow { IllegalArgumentException("Route not found: ${variant.routeId}") }
    ...
    override fun findAll(): List<RouteVariant> =
