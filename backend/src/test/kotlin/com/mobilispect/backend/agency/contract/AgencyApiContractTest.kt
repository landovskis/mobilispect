 ...
 *
 * Task T109-T110: Verify API responses match expected contract structure
 * for regional transit frequency overview functionality.
 *
 * Tests ensure:
...
class AgencyApiContractTest {
    ...
    private val agencyQueryService: AgencyQueryService = org.mockito.Mockito.mock(AgencyQueryService::class.java)

    private val mockMvc: MockMvc = MockMvcBuilders.standaloneSetup(
        AgencyController(agencyQueryService)
    ...
    fun `GET agencies matches contract`() {
        ...
        `when`(agencyQueryService.getAgencies(any())).thenReturn(
            PageImpl(listOf(agencyDTO), PageRequest.of(0, 20), 1)
        )

        val response = mockMvc.get("/api/agencies") {
    ...
    fun `GET agencies by region matches contract`() {
        val agencyDTO = AgencyDTO(
            ...
            routesByType = mapOf(
                RouteType.BUS to 15,
                RouteType.RAIL to 8,
                RouteType.FERRY to 2
            )
        )

        `when`(agencyQueryService.getAgenciesByRegion(any(), any())).thenReturn(
            PageImpl(listOf(agencyDTO), PageRequest.of(0, 20), 1)
        )

        val response = mockMvc.get("/api/regions/r-bay-area/agencies") {
            accept(MediaType.APPLICATION_JSON)
            param("page", "0")
            param("size", "20")
        }.andReturn().response
    ...
    fun `GET agency summary matches contract`() {
        val agencySummary = AgencySummaryDTO(
            id = "o-789",
        ...
        val response = mockMvc.get("/api/agencies/o-789") {
            accept(MediaType.APPLICATION_JSON)
        }.andReturn().response
    ...
    fun `GET agency summary returns 200 with null for non-existent agency`() {
        ...
        val response = mockMvc.get("/api/agencies/o-nonexistent") {
            accept(MediaType.APPLICATION_JSON)
        }.andReturn().response
    ...
    fun `GET agencies by region supports pagination`() {
        ...
        `when`(agencyQueryService.getAgenciesByRegion(any(), any())).thenReturn(
            PageImpl(listOf(agency1, agency2), PageRequest.of(0, 2), 2)
        )

        val response = mockMvc.get("/api/regions/r-test/agencies") {
            accept(MediaType.APPLICATION_JSON)
            param("page", "0")
            param("size", "2")
        }.andReturn().response
