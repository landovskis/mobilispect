...
internal class GTFSRouteDataSourceTest {
  ...
  private val agencyIDDataSource =
    StubAgencyIDDataSource(mapOf("CITPI" to "o-f256-exo~citlapresquîle"))
  private val routeIDDataSource =
    StubRouteIDDataSource(mapOf("1" to "r-f2566-1", "T1" to "r-f2566-t1", "115" to "r-f2565-115"))
  ...
  fun corrupted(@TempDir root: Path) {
    resourceLoader.copyResourceTo(
      src = "classpath:gtfs/routes/corrupt.txt",
      root = root,
      dst = "routes.txt",
    )
  ...
  fun importsSuccessfullyWithAgencyID(@TempDir root: Path) {
    resourceLoader.copyResourceTo(
      src = "classpath:gtfs/routes/full.txt",
      root = root,
      dst = "routes.txt",
    )
