...
class GtfsRtIngestionServiceTest {
  ...
  fun `ingest updates only lastCheckedAt when data is unchanged`() {
    val result =
      GtfsRtFetchResult.Unchanged(feedId, UnchangedReason.HTTP_NOT_MODIFIED, Instant.now())
    every { feedRepository.findByStatusAndRealtimeFeedUrlNotNull(FeedStatus.ACTIVE) } returns
      listOf(feed)
    coEvery { fetcher.fetchAllFeeds(listOf(feed)) } returns flowOf(result)
  ...
  @Test
  fun `ingest records circuit breaker states after each cycle`() {
    val result = newDataResult(feedId)
    every { feedRepository.findByStatusAndRealtimeFeedUrlNotNull(FeedStatus.ACTIVE) } returns
      listOf(feed)
    coEvery { fetcher.fetchAllFeeds(listOf(feed)) } returns flowOf(result)
    ...
    // Ensure a circuit breaker exists for the feed
    circuitBreakerRegistry.getOrCreate(feedId)

    ingestionService.ingest()

    val gauge =
      meterRegistry.find("gtfsrt.circuitbreaker.state").tag("feed_id", feedId.value).gauge()
    assertEquals(1.0, gauge?.value())
  }

  private fun newDataResult(feedId: FeedId) =
  ...
  private fun createFeed(feedId: FeedId) =
    Feed(
