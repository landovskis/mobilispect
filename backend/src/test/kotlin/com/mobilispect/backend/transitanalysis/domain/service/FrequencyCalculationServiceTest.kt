 ...
 *
 * FrequencyCalculationService calculates transit frequency (headway) metrics for route variants
 ...
 *
 * IMPORTANT: FrequencyCalculationService does NOT exist yet. Implementation comes AFTER these tests
 * are written.
 */
...
class FrequencyCalculationServiceTest {
   ...
   *
   * FR-008: Calculate average headway for variant and time period FR-012: Headway = time between
   * consecutive trips
   *
   * Example: Trips at 8:00, 8:15, 8:30, 8:45 Headways: 15min, 15min, 15min Average: 15 minutes
   */
  @Test
  fun `calculateFrequency() computes average headway from trip times`() {
    ...
    val tripTimes =
      listOf(LocalTime.of(8, 0), LocalTime.of(8, 15), LocalTime.of(8, 30), LocalTime.of(8, 45))
   ...
   * FR-009: Calculate min/max headway values
   *
   * Example: Trips at 8:00, 8:10, 8:25, 8:50 Headways: 10min, 15min, 25min Min: 10min, Max: 25min,
   * Average: 16.67min
   */
  @Test
   ...
   * FR-010: Mark irregular schedules (no fixed pattern)
   *
   * An irregular schedule is one where headway variance exceeds a threshold, indicating no
  ...
  fun `calculateFrequency() correctly classifies time periods`() {
    ...
    // AM Peak: 6:00-9:00
    val amPeakTrips = listOf(LocalTime.of(7, 0), LocalTime.of(7, 30), LocalTime.of(8, 0))

    // PM Peak: 16:00-19:00
    val pmPeakTrips = listOf(LocalTime.of(17, 0), LocalTime.of(17, 30), LocalTime.of(18, 0))

    // Off-Peak: Other hours
    val offPeakTrips = listOf(LocalTime.of(10, 0), LocalTime.of(14, 0))
  ...
  fun `calculateFrequency() classifies weekends correctly`() {
    ...
    val tripTimes = listOf(LocalTime.of(10, 0), LocalTime.of(10, 30), LocalTime.of(11, 0))
   ...
   *
   * Verifies that each frequency record has a timestamp indicating when the calculation was
   * performed.
   */
  @Test
   ...
   *
   * Verifies that only one frequency record exists per (variant, date, period). Attempting to
