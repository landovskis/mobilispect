 ...
 *
 * VariantIdentificationService identifies unique route variants by analyzing trip stop patterns and
 * generating deterministic SHA-256 hashes for variant IDs.
 *
 ...
 *
 * IMPORTANT: VariantIdentificationService does NOT exist yet. Implementation comes AFTER these
 * tests are written.
 */
...
class VariantIdentificationServiceTest {
   ...
   * FR-007: Use SHA-256 hash of stop pattern as variant identifier
   *
   * Verifies that the service generates a 64-character hexadecimal SHA-256 hash from the ordered
   * stop pattern.
   */
  @Test
   ...
   *
   * Verifies that identical stop patterns always generate the same hash, ensuring variant stability
   * across multiple feed imports.
   */
  @Test
   ...
   * FR-009: Different stop patterns produce different hashes
   *
   * Verifies that even minor differences in stop patterns (order, stops) produce completely
   * different hashes.
   */
  @Test
   ...
   *
   * Verifies that stop pattern order is significant - same stops in different order are different
   * variants.
   */
  @Test
   ...
   * FR-006: Identify route variants by unique stop sequences
   *
   * Verifies that multiple trips with the same stop pattern are grouped into a single variant, not
   * duplicate variants.
   */
  @Test
   ...
   *
   * Verifies that the service correctly identifies terminal stops for efficient querying (used in
