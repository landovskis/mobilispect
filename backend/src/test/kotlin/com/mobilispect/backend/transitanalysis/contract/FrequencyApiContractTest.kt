...
class FrequencyApiContractTest {
    ...
    private val frequencyQueryService: FrequencyQueryService = org.mockito.Mockito.mock(FrequencyQueryService::class.java)
    private val commonSectionService: CommonSectionService = org.mockito.Mockito.mock(CommonSectionService::class.java)
    ...
    fun `GET route matches contract`() {
        ...
        val response = mockMvc.get("/api/v1/routes/r-1") {
    ...
    fun `GET variants matches contract`() {
        ...
        val response = mockMvc.get("/api/v1/routes/r-1/variants") {
            accept(MediaType.APPLICATION_JSON)
        }.andReturn().response
    ...
    fun `GET variant frequencies matches contract`() {
        ...
        val response = mockMvc.get("/api/v1/routes/variants/${"a".repeat(64)}/frequencies") {
            accept(MediaType.APPLICATION_JSON)
        }.andReturn().response
    ...
    fun `GET common sections matches contract`() {
        ...
        val response = mockMvc.get("/api/v1/common-sections/routes/r-1") {
            accept(MediaType.APPLICATION_JSON)
        }.andReturn().response
    ...
    fun `GET common section frequency matches contract`() {
        ...
        `when`(commonSectionService.getCombinedFrequency(eq(id), eq(TimePeriod.WEEKDAY_AM_PEAK))).thenReturn(
        ...
        val response = mockMvc.get("/api/v1/common-sections/$id/frequency?timePeriod=WEEKDAY_AM_PEAK") {
            accept(MediaType.APPLICATION_JSON)
        }.andReturn().response
