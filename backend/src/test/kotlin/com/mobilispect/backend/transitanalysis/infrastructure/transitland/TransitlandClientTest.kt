 ...
 *
 * These tests verify that the existing TransitLandClient (from schedule module) provides
 ...
 *
 * IMPORTANT: These tests use @Disabled because they require a valid Transit.land API key
 * and make real HTTP calls. They serve as integration tests and documentation.
 * Unit tests for the transit analysis module will mock the TransitLandAPI interface.
 */
class TransitlandClientTest {
     ...
     * Requirement FR-001b: System MUST import region definitions from Transitland API metro areas
     *
     * This test verifies that the TransitLandClient provides the operators() method
     * which is essential for discovering transit agencies across regions.
     */
    @Test
     ...
     *
     * Constitutional Requirement SC-005: System processes feed data for a metropolitan
     * region containing up to 20 agencies within 5 minutes
     *
     * This test verifies that pagination works correctly for fetching all operators
     ...
     *
     * User Story 4 (US4): Import and Process Regional Transit Data
     * Requirement FR-001a: System MUST query Transitland API to discover GTFS feed URLs
     *
     * This test verifies that feedMetadata() provides all necessary feed details
     * including download URLs, version information, and authorization requirements.
     */
    @Test
    @Disabled("Requires valid Transit.land API key - enable for integration testing")
     ...
     *
     * Constitutional Requirement: API client MUST enforce rate limiting
     * Transit.land API Requirement: Maximum 6 requests per second
     *
     * This test verifies that the rate limiter correctly throttles requests
     * to comply with Transit.land API limits.
     */
    @Test
     ...
     *
     * This test verifies that API errors (401 Unauthorized, 429 Too Many Requests)
     * are properly handled and returned as Result failures.
     */
    @Test
     ...
     *
     * This test verifies that when the Transit.land API returns 429 Too Many Requests,
     * the client properly handles it and returns an appropriate error.
     */
    @Test
     ...
     *
     * The TransitLandClient uses a Semaphore (limit: 6) to control concurrent requests.
     * This test documents that concurrency control exists and works correctly.
     */
    @Test
     ...
     *
     * This test verifies that requesting metadata for a non-existent feed
     * returns a clear failure rather than throwing an exception.
     */
    @Test
    ...
    fun `feedMetadata() returns failure for non-existent feed`() {
        ...
        result.onFailure { error ->
            assertThat(error.message).contains("No feed data found")
        }
    }
}
