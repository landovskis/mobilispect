...
class RegionControllerTest {
  ...
  fun `listRegions returns all regions when no filter applied`() {
    ...
    every { feedRepository.findAllByRegionRegionOnestopId(RegionId("r-region-1")) } returns
      emptyList()
    every { feedRepository.findAllByRegionRegionOnestopId(RegionId("r-region-2")) } returns
      emptyList()
    ...
    assertThat(response.regions.map { it.regionOnestopId })
  ...
  fun `listRegions includes feed count in response`() {
    ...
    val feeds = listOf(createFeed("f-feed-1", region), createFeed("f-feed-2", region))
  ...
  fun `listRegions filters by hasImportedFeeds`() {
    ...
    // When
    val response = controller.listRegions(autoUpdateEnabled = null, hasImportedFeeds = true)
  ...
  fun `listRegions combines hasImportedFeeds and autoUpdateEnabled filters`() {
    ...
    every { regionRepository.findAllByAutoUpdateEnabledWithCompletedImports(true) } returns
      listOf(region)
    every { feedRepository.findAllByRegionRegionOnestopId(any()) } returns emptyList()

    // When
    val response = controller.listRegions(autoUpdateEnabled = true, hasImportedFeeds = true)
  ...
  fun `listRegions with hasImportedFeeds false returns all regions`() {
    ...
    // When
    val response = controller.listRegions(autoUpdateEnabled = null, hasImportedFeeds = false)
  ...
  fun `listRegions with hasImportedFeeds null uses default behavior`() {
    // Given
    val regions = listOf(createRegion("r-region-1", "Region 1", autoUpdate = true))
  ...
  fun `listRegions with autoUpdateEnabled true and hasImportedFeeds false uses only autoUpdate filter`() {
    ...
    // When
    val response = controller.listRegions(autoUpdateEnabled = true, hasImportedFeeds = false)
