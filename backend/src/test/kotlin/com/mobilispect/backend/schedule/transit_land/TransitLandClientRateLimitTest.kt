...
class TransitLandClientRateLimitTest {
  ...
  fun `rate limiter enforces 6 requests per second limit`() {
    ...
    val timeTaken = measureTimeMillis { repeat(10) { RateLimiter.waitForPermission(rateLimiter) } }
  ...
  fun `rate limiter respects Transit land limit of 6 per second`() {
    ...
    val timeTaken = measureTimeMillis { repeat(6) { RateLimiter.waitForPermission(rateLimiter) } }
