...
def resolve_region_id(region_id: Optional[str], region_name: Optional[str]) -> str:
    if region_id:
        return region_id
    if not region_name:
        raise ValueError("region_id or region_name is required")

    target = _normalize_text(region_name)
    engine = get_engine()
    ...
    matches = []
    for row in rows:
        name_normalized = _normalize_text(row.get("name"))
        if not name_normalized:
            continue
        if (
            name_normalized == target
            or target in name_normalized
            or name_normalized in target
        ):
...
def discover_region_feeds(region_id: str) -> Dict[str, int]:
    ...
    while True:
        params = {"limit": 100, city_name: region_row["name"]}
