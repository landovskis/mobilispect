...
export class DiscoverRegionsPageComponent implements OnInit, OnDestroy {
  ...
  importRegion(): void {
    ...
    if (feeds.length === 0) {
      this.snackBar.open(
        `No active feeds available for ${regionName}`,
        'Close',
        { duration: 3000 },
      );
      return;
    ...
    this.snackBar.open(
      `Starting import for ${feeds.length} feeds in ${regionName}...`,
      'Close',
      { duration: 2000 },
    );
    ...
    const requests = feeds.map((feed) =>
      this.importService.startImport(feed.feedOnestopId).pipe(
        ...
        catchError((error) =>
          of({ feed, ok: false, error } as RegionImportResult),
        ),
      ),
    );
    ...
    forkJoin(requests)
      ...
      .subscribe((results) => {
        const failed = results.filter(
          (result): result is RegionImportResult & { ok: false } => !result.ok,
        );
        const successCount = results.length - failed.length;
        if (failed.length > 0) {
          const errorPayload = failed[0].error as
          ...
          this.snackBar.open(
            `Imports started for ${feeds.length} feeds in ${regionName}`,
            'Close',
            { duration: 3000 },
          );
        }
