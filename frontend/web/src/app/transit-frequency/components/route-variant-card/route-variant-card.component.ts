...
@Component({
  ...
  template: `
    ...
    >
      <app-brand-card [loading]="loading" title="{{ stopRangeLabel }}">
        @if (!loading && variant) {
              ...
              @for (stopName of stopNames; track $index) {
                ...
                @if (spacingByIndex[$index]) {
                  <li class="spacing-inline">
                    <span
                      class="spacing-segment rounded-full px-2 py-0.5 text-[0.75rem] font-semibold"
                    >
  ...
  styles: [
    `
      ...
      .classification.region-local {
        background: rgba(14, 165, 233, 0.15);
        color: #0369a1;
      }
      .classification.region-rapid {
        background: rgba(37, 99, 235, 0.18);
        color: #1e3a8a;
      }
      .classification.region-express {
      ...
      :host-context(.dark-theme) .classification.region-local {
        background: rgba(56, 189, 248, 0.25);
        color: #bae6fd;
      }
      :host-context(.dark-theme) .classification.region-rapid {
        background: rgba(96, 165, 250, 0.25);
        color: #bfdbfe;
      }
      :host-context(.dark-theme) .classification.region-express {
...
export class RouteVariantCardComponent {
  ...
  get spacingByIndex(): string[] {
    return (
      this.variant?.stopSpacingMeters?.map((meters: number) =>
        meters.toFixed(0),
      ) ?? []
    );
  }
  ...
  get averageSpacingMeters(): string {
    if (
      !this.variant ||
      this.variant.averageStopSpacingMeters === null ||
      this.variant.averageStopSpacingMeters === undefined
    ) {
      return '—';
    }
