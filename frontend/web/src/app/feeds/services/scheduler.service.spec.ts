...
describe('SchedulerService', () => {
  ...
  it('converts scheduler status dates', () => {
    ...
    const req = httpMock.expectOne(`${environment.apiUrl}/api/feeds/scheduler/status`);
    req.flush({
      enabled: true,
      totalActiveFeeds: 2,
  ...
  it('converts import stats and version timestamps', () => {
    ...
    const statsReq = httpMock.expectOne(`${environment.apiUrl}/api/feeds/scheduler/stats`);
    statsReq.flush({
    ...
    const versionsReq = httpMock.expectOne(`${environment.apiUrl}/api/feeds/scheduler/versions`);
    versionsReq.flush([
      {
  ...
  it('handles null scheduler timestamps and config updates', () => {
    ...
    const statusReq = httpMock.expectOne(`${environment.apiUrl}/api/feeds/scheduler/status`);
    statusReq.flush({
      enabled: true,
      totalActiveFeeds: 2,
      feedsCheckedInLast24Hours: 1,
    ...
    const configReq = httpMock.expectOne(`${environment.apiUrl}/api/feeds/scheduler/config`);
    configReq.flush({
    ...
    const updateReq = httpMock.expectOne(`${environment.apiUrl}/api/feeds/scheduler/config`);
  ...
  it('loads single feed versions and refreshes', () => {
    ...
    const feedReq = httpMock.expectOne(`${environment.apiUrl}/api/feeds/scheduler/versions/f-1`);
    feedReq.flush({
      feedOnestopId: 'f-1',
      currentVersionSha1: 'abc',
      latestVersionSha1: 'def',
    ...
    const refreshReq = httpMock.expectOne(`${environment.apiUrl}/api/feeds/scheduler/versions/f-1/refresh`);
  ...
  it('triggers manual checks and toggles auto update', () => {
    ...
    const manualReq = httpMock.expectOne(`${environment.apiUrl}/api/feeds/scheduler/manual-check`);
    ...
    const enableReq = httpMock.expectOne(`${environment.apiUrl}/api/feeds/scheduler/feeds/f-2/auto-update/enable`);
    expect(enableReq.request.method).toBe('POST');
    enableReq.flush({});

    service.disableFeedAutoUpdate('f-2').subscribe();
    const disableReq = httpMock.expectOne(`${environment.apiUrl}/api/feeds/scheduler/feeds/f-2/auto-update/disable`);
  ...
  it('maps feed update checks', () => {
    ...
    const req = httpMock.expectOne(`${environment.apiUrl}/api/feeds/scheduler/feeds/f-abc-test/check-update`);
