...
describe('ImportUtils', () => {
  ...
  it('detects active and completed states', () => {
    ...
    const completed: FeedImport = { ...baseImport, status: ImportStatus.COMPLETED };
  ...
  it('formats status metadata', () => {
    expect(ImportUtils.isSuccessful({ ...baseImport, status: ImportStatus.COMPLETED })).toBeTrue();
    expect(ImportUtils.isCancellable({ ...baseImport, status: ImportStatus.RUNNING })).toBeTrue();
    expect(ImportUtils.isCancellable({ ...baseImport, status: ImportStatus.FAILED })).toBeFalse();
  ...
  it('formats estimated remaining time and relative timestamps', () => {
    ...
    expect(ImportUtils.formatRelativeTime(new Date('2024-01-01T11:59:30Z').toISOString())).toBe('Just now');
    expect(ImportUtils.formatRelativeTime(new Date('2024-01-01T11:30:00Z').toISOString())).toBe('30m ago');
    expect(ImportUtils.formatRelativeTime(new Date('2023-12-30T12:00:00Z').toISOString())).toBe('2d ago');
  });
});
...
describe('ProgressUtils', () => {
  it('clamps progress values and handles failures', () => {
    ...
    expect(ProgressUtils.getProgressPercentage({
    ...
    expect(ProgressUtils.isProgressFailed({
      progressPercentage: -1,
      totalSteps: 100,
      currentStep: 'FAILED',
      estimatedTimeRemainingSeconds: null,
  ...
  it('returns color classes for progress thresholds', () => {
    ...
    expect(ProgressUtils.getProgressColorClass({
      progressPercentage: -1,
      totalSteps: 100,
      currentStep: 'FAILED',
      estimatedTimeRemainingSeconds: null,
