    ...
    <div class="tab-content py-6 max-md:py-4">
      @if (activeImports$ | async; as activeImports) {
        @if (activeImports.length > 0) {
          <div
            class="bulk-actions flex items-center gap-4 border-b border-[#e0e0e0] bg-[#f5f5f5] px-6 py-4 max-md:flex-col max-md:items-start max-md:gap-3"
          >
            <mat-checkbox
        ...
        @if (activeImports.length === 0) {
          <div class="no-imports px-5 py-10 text-center text-[#666]">
            <mat-icon class="no-imports-icon mb-4 text-[48px] text-[#ccc]"
