...
@Component({
  ...
  standalone: true,
  imports: [MatIconModule, BrandCardComponent, BrandButtonComponent],
  template: `
    <div class="tab-content py-6 max-md:py-4">
      <app-brand-card class="welcome-card mb-6">
        <p>
          Choose from available metropolitan regions to view and import their
          transit feeds.
        </p>
      </app-brand-card>
      ...
      @if (regions?.length) {
          ...
          @for (region of regions!; track region.regionOnestopId) {
            ...
            >
              <div class="flex items-center justify-between gap-3">
                <div>
                  <div
                    class="text-lg font-semibold text-[var(--ms-color-primary,#0b4f8a)]"
                  >
                    {{ region.name }}
                  </div>
                  <div class="text-[var(--ms-color-muted,#6b7280)]">
                    {{ region.feedCount }} feeds
                  </div>
                </div>
                <mat-icon
                  class="!text-3xl text-[var(--ms-color-primary,#0b4f8a)]"
                  >public</mat-icon
                >
              </div>
        ...
        <div class="empty-state mt-6">
          <div
            class="empty-content flex flex-col items-center px-5 py-10 text-center text-[#666]"
          >
            <mat-icon class="empty-icon mb-4 text-[48px] text-[#ccc]"
              >location_off</mat-icon
            >
            <h3 class="mb-2 font-medium m-0">No regions available</h3>
            <p class="text-[#999] m-0">
              Try refreshing data or check your connection.
            </p>
          </div>
        </div>
      }
