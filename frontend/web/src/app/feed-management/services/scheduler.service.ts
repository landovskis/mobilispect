...
export class SchedulerService {
  private readonly apiUrl = `${environment.apiUrl}/api/feed-management/scheduler`;
  ...
  getImportStats(): Observable<ImportStats> {
    return this.http.get<ImportStats>(`${this.apiUrl}/stats`)
      .pipe(
        map(stats => ({
          ...stats,
  ...
  getAllFeedVersions(): Observable<FeedVersionInfo[]> {
    return this.http.get<FeedVersionInfo[]>(`${this.apiUrl}/versions`)
      .pipe(
  ...
  getFeedVersion(feedOnestopId: string): Observable<FeedVersionInfo> {
    return this.http.get<FeedVersionInfo>(`${this.apiUrl}/versions/${feedOnestopId}`)
      .pipe(
        map(version => ({
          ...version,
          lastCheckedAt: version.lastCheckedAt ? new Date(version.lastCheckedAt) : undefined,
  ...
  refreshFeedVersion(feedOnestopId: string): Observable<FeedVersionInfo> {
    return this.http.post<FeedVersionInfo>(`${this.apiUrl}/versions/${feedOnestopId}/refresh`, {})
      .pipe(
        map(version => ({
          ...version,
          lastCheckedAt: version.lastCheckedAt ? new Date(version.lastCheckedAt) : undefined,
  ...
  checkFeedUpdate(feedOnestopId: string): Observable<boolean> {
    return this.http.get<{ hasUpdate: boolean }>(`${this.apiUrl}/feeds/${feedOnestopId}/check-update`)
