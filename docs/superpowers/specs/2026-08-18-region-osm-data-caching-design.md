...
**Verified post-implementation (still partially open):** the file is a national ZIP (all CMA/CAs in one file, cached once and reused for every region), Shapefile format, projected in NAD83 / Statistics Canada Lambert (**EPSG:3347**) — not WGS84 — so the reprojection step above is required, not optional. Confirmed DBF attribute fields: `CMANAME` (name, char 100), `CMAUID`, `CMATYPE`, `CMAPUID`, `PRUID`. `LAMBERT_PROJ4`'s exact numeric parameters were confirmed correct via search against spatialreference.org/epsg.io. The exact CMA/CA cartographic boundary shapefile zip filename remains ambiguous — `statcan.gc.ca` and `geo.statcan.gc.ca` are both blocked by every sandbox this spec and its implementation were written in, including via direct fetch of the ArcGIS `MapServer` JSON, so it could never be confirmed by rendering the real page (unlike the Overpass query shape in the referenced OSM-import plan, which was). Two candidate filenames turned up in search results (`lcma000b21a_e.zip`, matching the historical `a` = ArcInfo/shapefile format-code letter used since at least the 2016 census; `lcma000b21s_e.zip`, matching the `s`-suffixed layer names on StatsCan's live 2021 `Cartographic_boundary_files` ArcGIS service, though that may be an internal REST layer-naming convention unrelated to the downloadable zip). The implementation tries both, in that order, rather than guessing one (`crates/worker/src/region_provisioning/statcan.rs`'s `download_statcan_zip`) — confirm the real filename and simplify back to one URL whenever this runs somewhere with actual network access to the StatsCan site.
